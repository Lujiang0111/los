...
bool Sockaddr6::Bind(int fd, SockaddrInterface *local_addr)
{
    ...
    if (local_addr6)
    {
        ...
        {
            los::logs::Printfln("bind fail! ip=%s, port=%hu, error=%d", local_addr6->ip_.c_str(), local_addr6->port_, los::socks::GetLastErrorCode());
            return false;
        }
    }
    else
